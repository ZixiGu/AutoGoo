/**
 * AutoGoo constants — interaction templates, reference rules, prompt snippets.
 *
...
export const TEMPLATE_PROJECT_WORKSPACE_CREATE: InteractionTemplate = {
  ...
  options: [
    { label: "不创建 (Recommended)", description: "只初始化 .goo/ AutoGoo 状态目录，不新增 src/data/docs 等业务目录。", value: "no" },
...
export const TEMPLATE_PROJECT_WORKSPACE_CLAUDE_MD: InteractionTemplate = {
  ...
  options: [
    { label: "是 (Recommended)", description: "在 CLAUDE.md 的 AutoGoo marker 段写入目录结构和用途。", value: "yes" },
...
- 实现完成后运行验证命令确认正确性`,
  ...
  optimizer: `你是 AutoGoo Optimizer。你的任务是性能优化和效率提升。
- 先建立指标和基线，再做改动
...
- 分析失败案例，提出改进建议`,
  ...
  reviewer: `你是 AutoGoo Reviewer。你的任务是代码审查和方案评审。
- 检查代码正确性、安全性和性能
- 验证是否满足验收标准
- 指出潜在问题并给出改进建议
- 输出审查报告`,

  auditor: `你是 AutoGoo Auditor。你的任务是证据审计和合规检查。
- 检查步骤产物是否完整
- 验证日志、产物路径和验收结果的一致性
- 检查是否遵循了项目约束和规范
- 输出审计报告`,
...
export const AUTOGOO_SYSTEM_PROMPT = `
## AutoGoo DAG 工作流框架

AutoGoo 是一个 DAG 驱动的多智能体编排框架。收到复杂多步任务后：

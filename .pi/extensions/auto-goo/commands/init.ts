...
export async function handleGooInit(args: string, ctx: ExtensionContext): Promise<void> {
  ...
  if (scope === "project") {
    ...
    if (needServers) {
      servers = await collectServers(ctx);
    }
...
async function collectServers(ctx: ExtensionContext): Promise<ServerInfo[]> {
  ...
  while (addMore) {
    ...
    const name = await uiInput(ctx, "服务器名称/别名", defaultName) || defaultName;
    ...
    servers.push({ name, host, user, port, type, purpose });

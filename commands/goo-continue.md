...
10. 如果配置中存在 `servers[]`，且当前 thread 没有 `runtime.remote_resource_decision`，或用户明确要求重新选择执行位置，先运行 `skills/auto-goo/scripts/remote-resources.py --probe` 展示 CPU/内存/磁盘/GPU 摘要，并用 `AskUserQuestion` 复用 `id=remote_resource_usage` 模板询问是否把本次待执行步骤放到远程服务器。用户选择本地或未明确确认时保持本地执行；用户选择服务器后，才更新相关待执行 step 的 `execution_target`、`remote_server`、`remote_reason` 和 `requires_user_confirm=true`。
...
14. 按 AutoGoo 标准执行流程继续（Phase 2-4）
